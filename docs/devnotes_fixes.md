...
**Fix:** Replace with plain absolute values for `APILOGICPROJECT_APILOGICSERVERHOME` (required by the server), and set the venv variables directly:
...
**Also updated:** `"python.defaultInterpreterPath"` from:
```
...
```
This is required because the project is not in the standard location one level below the Manager.

---

## Fix 3 — `.vscode/launch.json`: Pin interpreter in the ApiLogicServer launch config

**Problem:** Even after updating `settings.json`, VS Code's debugpy launch uses the interpreter from the workspace interpreter picker (cached separately from `python.defaultInterpreterPath`). If the picker still shows the wrong Python, F5 fails at `import flask_sqlalchemy` (line 42 of `api_logic_server_run.py`) because the system Python has no project packages.
...
The `"python"` key in a debugpy launch entry overrides the workspace interpreter selection entirely. This guarantees F5 always uses the correct venv regardless of the interpreter picker state.
...
|------|--------|------------|
| `.env` | Removed bash variable substitution; switched to absolute path | `.env` is not a shell script |
| `.vscode/settings.json` | Removed custom terminal profile; updated interpreter path to absolute | Space in workspace path broke `source` inside `-c` string |
| `.vscode/launch.json` | Added `"python"` key pinning the venv interpreter | Interpreter picker cached wrong Python; `"python"` key overrides it |

---

## Portability note

Fixes 2 and 3 embed an absolute Mac path. For a Windows colleague:
...
If the project were in the standard location (one level below the Manager), all three would use `${workspaceFolder}/../venv/...` and would be portable without changes.
